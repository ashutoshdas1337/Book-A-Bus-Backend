...
const createUser = async (req, res) => {
  ...
  try {
    if (!username || !email || !password || !contact)
      return res.status(400).json({ message: "Please provide full info" });
    const checkEmail = await User.findOne({ email });
    const checkPhone = await User.findOne({ contact });
    if (checkEmail || checkPhone)
      return res
        .status(409)
        .json({ message: "email or password already registered" });
...
const authorizeUser = async (req, res) => {
  ...
  try {
    ...
    if (!checkEmail)
      return res
        .status(404)
        .json({ message: "User does not exist, consider creating one" });
    ...
    return res
      .status(500)
      .json({ message: "error occured while logging user " });
  }
};
