...
await redis.setEx(cacheKey,60,JSON.stringify(getTicket_fromMongo))
    return res.status(200).json({message:"ticket retrieved (from mongo)",retrieved:getTicket_fromMongo})
} catch (error) {
console.log(error)
return res.status(500).json({message:"error retrieving ticket"})    
}
}
router.get("/showUserTicket",tokenChecker,showTicket)
